module EventCalendar
  ...
  module ClassMethods
    ...
    def event_strips_for_month(shown_date, course_id, first_day_of_week=0)
      strip_start, strip_end = get_start_and_end_dates(shown_date, first_day_of_week)
      events = events_for_date_range(strip_start, strip_end, course_id)
    ...
    def get_start_and_end_dates(shown_date, course_id, first_day_of_week=0)
    ...
    def events_for_date_range(start_d, end_d, course_id)
      self.find(
        :all,
        :conditions => [ '(? <= end_at) AND (start_at < ?) AND course_id = ?', start_d.to_time.utc, end_d.to_time.utc, course_id ],
        :order => 'start_at ASC'
      )
    end
